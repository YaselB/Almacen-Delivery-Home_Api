...
public class UpdateComboEntityCommandHandler : UpdateGenericEntityCommandHandler<ComboEntity, UpdateComboEntityCommand>
{
    ...
    public UpdateComboEntityCommandHandler(IComboRepository repository, IMapper mapper , ILogger<ComboEntity> logger , IHistoryRepository historyRepository , IProductRepository product , IProductComboRepository productCombo) : base(repository, mapper)
    {
        this.logger = logger;
    ...
    public override async Task<Result<Unit>> Handle(UpdateComboEntityCommand request, CancellationToken cancellationToken)
{
    ...
    var history = HistoryEntity.Create(HistoryEntity.Type.Modificaciones, combo.Name, message);
